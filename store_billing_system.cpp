...
void shopping :: menu()
{
	...
	switch(choice)
	{
		case 1:
			...
			if(email=="a@email.com" && password=="a@123")
			{
				administrator();
			}
			else
...
void shopping ::add()
{
	...
	data.open("billingdata.txt",ios::in);
	
	if(!data)
	{
		data.open("billingdata.txt", ios::app|ios::out);
...
void shopping ::edit()
{
	...
	if(!data)
	{
		...
		data1.open("TEMbillingdata.txt", ios::app|ios::out);
		...
		remove("billingdata.txt");
		rename("TEMbillingdata.txt","billingdata.txt");
